...
DESCRIPTION:
            This program is a wrapper for linear regression performed by
            statsmodels. In addition to returning a regression model, the
            program can report virtually every intermediary and end result of
            that computation. The program uses statsmodels, but the user does
            not need to know how to use that package. Instead, the main function,
            linreg() takes two parameters, X and y, and computes everything most
            users would care to know, and more.

            The purpose of this module is not speed or versatility. The goals
            are very restricted and are:
                 (1) Provide easy access to linear regression analysis (see
                 "EXAMPLE USAGE" below). X and y data can be passed to linreg()
                 in several different formats, described in detail below. "Easy"
                 means there is a single command that takes one or more "X"
                 variable(s) and a "y" variable. "Easy" also means that r_linreg
                 can take "X" and "y" in flexible formats.

                 (2) Underlying parameters generated by regression analysis can
                 be retrieved easily. See definitions() or calculations() for a
                 complete list of statistical parameters.

      NOTES:
            (1) Other forms of regression including Logistic Regression,
            Stepwise Regression, Ridge Regression, Lasso Regression, and
            ElasticNet Regression are not addressed in this module.
...
STATSMODELS:
    ...
    model =  ols(formula="Lottery ~ Literacy + Wealth + Literacy*Wealth", data=df)
        ...
        where: df is a pandas DataFrame containing Literacy, Wealth (X columns),
        and Lottery (y) columns.
